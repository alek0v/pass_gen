...
def guest_prov(q):  # Перевірка правильності введення вибору
    while True:
...
def num_prov(num):  # Перевірка чи введено число
    while True:
        if not num.isdigit():
...
def gen_pass(bloknot, lenn):  # Функція генерації пароля
    out = ''
    for i in range(lenn):
        out += choice(bloknot)
    return out
...
if guest_prov(input('Сделать пароль сложным? ')):
    bloknot += digits + lowercase_letters + uppercase_letters + punctuation  # Формуємо блокнот
else:
    if guest_prov(input('Включать ли цифры 0123456789? ')):  # Якщо потрібен не складний пароль - формуємо блокнот без деяких символів
        bloknot += digits
...
print()
for i in range(amount): # Генерація визначеної кількості паролів
